...
\begin{frame}
  \begin{block}{Goals}
    \begin{itemize}
      ...
      \item Implement optimizations aimed at improving the Rx/Tx packet rate when a virtual \emph{e1000} network adapter is used in a VM
    \end{itemize}
...
\frametitle{Introduction}
  ...
  \begin{block}{CPU Virtualization techniques}
    \begin{itemize}
      \item Interpretation: Doing in software what CPU does in hardware
...
\frametitle{Problems of the existing implementation}
  ...
  \only<3> {
    \begin{block}{What are the problems \& bottlenecks?}
      \begin{itemize}
...
\frametitle{The paravirtualization concept}
  \only<1> {
	...
	\item Advantage: The hypervisor can then support an unmodified OS
...
\frametitle{Porting the paravirtualization concept to e1000}
  ...
  \only<1> {
	...
	\item Consumer work is done asynchronously w.r.t. the consumer and with notifications disabled
...
\frametitle{Conclusions}
  \begin{block}
    \begin{itemize}
      \item Some simple modifications to the e1000 framework leads to big performance improvements
      \item 
    \end{itemize}
  \end{block}
\end{frame}
